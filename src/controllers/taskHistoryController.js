...
const controllerName = 'taskhistory'

router.get(`/${controllerName}`, async (req, res) => {
  let reponse
  try {
    if (req.headers.oauth) {
      reponse = await taskHistoryService.getAll()
...
router.get(`/${controllerName}ById`, async (req, res) => {
  ...
  try {
    if (req.headers.oauth) {
      reponse = await taskHistoryService.getById(req.body)
...
router.post(`/${controllerName}`, async (req, res) => {
  ...
  try {
    ...
    if (req.headers.oauth) {
      reponse = await taskHistoryService.postNew(req.body)
      const dateHour = moment().format()
...
router.delete(`/${controllerName}`, async (req, res) => {
  let reponse
  try {
    if (req.headers.oauth) {
      reponse = await taskHistoryService.deleteById(req.body)
      const dateHour = moment().format()
      if (reponse == "Ok") {
...
router.put(`/${controllerName}`, async (req, res) => {
  let reponse
  try {
    if (req.headers.oauth) {
    reponse = await taskHistoryService.putById(req.body)
    const dateHour = moment().format()
    if (reponse == "Ok") {
